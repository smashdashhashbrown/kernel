...
static ssize_t procfs_read(struct file *filp,
    ...
    *offset += procfs_buffer_size;
    ...
    pr_debug("[%s] procfs_read: %lu bytes\n", MOD_NAME, procfs_buffer_size);
...
static ssize_t procfs_write(struct file *file,
    ...
    *off += procfs_buffer_size;
    ...
    pr_debug("[%s] procfs_write: write %lu bytes\n", MOD_NAME, procfs_buffer_size);
    return procfs_buffer_size;
}
...
static int __init procfs3_init(void) {
    ...
    if (NULL == our_proc_file) {
        pr_debug("[%s] Error: Could not initialize /proc/%s\n", MOD_NAME, PROCFS_ENTRY_FILENAME);
        return -ENOMEM;
    }
    // Sets the size of a /proc file entry

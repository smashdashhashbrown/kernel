/*
 * print_string.c - Send output to the tty we're running on, regardless if it is
...
static void print_string(char *str) {
    ...
    if (my_tty) {
        ...
        /* my_tty->driver is a struct which holds the tty's functions, one
         * of which (write) is used to write strings to the tty.
         * It can be used to take a string either from the user's or kernel's
         * memory segment.
         *
         ...
         *
         * As you will see below, sometimes it's necessary to use preprocessor
         * stuff to create code that works for different kernel versions. The
         * (naive) approach we've taken here does not scale well. The right
         * way to deal with this is described in section 2 of 
